...
TEST(FIFOCacheTest, InitTest) {
  
    FifoCache<string,int> cache(42);
...
TEST(FIFOCacheTest, ReplaceValueInCache)
{
	FifoCache<int, int> cache(1);
...
TEST(FIFOCacheTest, GetValueFromCache)
{
	FifoCache<int, string> cache(1);
...
TEST(FIFOCacheTest, Evict)
{
    std::stringstream buffer;
    std::streambuf* old = std::cout.rdbuf(buffer.rdbuf());

	FifoCache<string, int> cache(4);
	int resp;
